...
const config: AuthRequestConfig = {
  ...
  redirectUri: makeRedirectUri({ scheme: 'hopnjob' }),
};
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => { handleAppleResponse(); }, [appleResponse]);
    ...
    const restoreSession = async () => {
      ...
      try {
        if (isWeb) {
          ...
          const sessionResponse = await fetch(`${BASE_URL}/api/auth/session`, { method: "GET", credentials: "include", }); // Important: This includes cookies in the request
          if (sessionResponse.ok) {
            ...
            catch (e) { console.log("Failed to refresh token on startup"); }
          }
        } else {
          ...
          console.log("Restoring session - Access token:", storedAccessToken ? "exists" : "missing");
          console.log("Restoring session - Refresh token:", storedRefreshToken ? "exists" : "missing");
  ...
  const refreshAccessToken = async (tokenToUse?: string) => {
    ...
    try {
      ...
      console.log("Current refresh token:", currentRefreshToken ? "exists" : "missing");
      if (isWeb) {
        ...
        const sessionResponse = await fetch(`${BASE_URL}/api/auth/session`, { method: "GET", credentials: "include", });
        if (sessionResponse.ok) {
        ...
        const refreshResponse = await fetch(`${BASE_URL}/api/auth/refresh`, {
          method: "POST",
          headers: { "Content-Type": "application/json", },
          body: JSON.stringify({ platform: "native", refreshToken: currentRefreshToken, }),
        });
        ...
        console.log("Received new access token:", newAccessToken ? "exists" : "missing");
        console.log("Received new refresh token:", newRefreshToken ? "exists" : "missing");
        if (newAccessToken) setAccessToken(newAccessToken);
        if (newRefreshToken) setRefreshToken(newRefreshToken);
        // Save both tokens to cache
        if (newAccessToken) await tokenCache?.saveToken("accessToken", newAccessToken);
        if (newRefreshToken) await tokenCache?.saveToken("refreshToken", newRefreshToken);
        ...
        if (newAccessToken) {
          ...
          if (!hasRequiredFields) console.warn("Refreshed token is missing some user fields:", decoded);
          setUser(decoded as AuthUser);
        }
      ...
      signOut(); // If there's an error refreshing, we should sign out
      return null;
    }
    finally { refreshInProgressRef.current = false; }
  };

  const handleNativeTokens = async (tokens: { accessToken: string; refreshToken: string; }) => {
    const { accessToken: newAccessToken, refreshToken: newRefreshToken } = tokens;
    console.log("Received initial access token:", newAccessToken ? "exists" : "missing");
    console.log("Received initial refresh token:", newRefreshToken ? "exists" : "missing");
    ...
    if (appleResponse?.type === "success") {
      try {
        ...
        const response = await exchangeCodeAsync(
          { clientId: "apple", code, redirectUri: makeRedirectUri(), extraParams: { platform: Platform.OS } },
          appleDiscovery
        );
  ...
  async function handleResponse() {
    ...
    if (response?.type === "success") {
      try {
        ...
        else { console.warn("No code verifier found in request object"); }
      ...
      catch (e) { console.error("Error handling auth response:", e); }
  ...
  const fetchWithAuth = async (url: string, options: RequestInit) => {
    if (isWeb) {
      ...
      if (response.status === 401) {
        ...
        if (user) { return fetch(url, { ...options, credentials: "include", }); }
      }
  ...
  const signIn = async () => {
    console.log("signIn");
    try {
      if (!request) { console.log("No request"); return; }
      await promptAsync();
    }
  ...
  const signInWithAppleWebBrowser = async () => {
    try {
      if (!appleRequest) { console.log("No appleRequest"); return; }
  ...
  const signOut = async () => {
    if (isWeb) {
      ...
      try { await fetch(`${BASE_URL}/api/auth/logout`, { method: "POST", credentials: "include", }); }
      catch (error) { console.error("Error during web logout:", error); }
    } else {
  ...
  return (
    <AuthContext.Provider
      value={{ user, signIn, signOut, signInWithApple, signInWithAppleWebBrowser, isLoading, error, fetchWithAuth, }}
    >
      {children}
    </AuthContext.Provider>
  );
