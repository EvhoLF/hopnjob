  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      try {
        ...
        if (platform === "native" && jsonBody.refreshToken) { refreshToken = jsonBody.refreshToken; }
      }
      catch (e) { console.log("Failed to parse JSON body, using default platform"); }
    } else if (
      ...
      catch (e) { console.log("Failed to parse form data, using default platform"); }
    } else {
      ...
      catch (e) { console.log("Failed to parse URL parameters, using default platform"); }
    }
        ...
        try {
          ...
          if (platform === "web") {
            ...
            response.headers.set(
              "Set-Cookie",
              `${COOKIE_NAME}=${newAccessToken}; Max-Age=${COOKIE_OPTIONS.maxAge
    ...
    const hasRequiredUserInfo = userInfo.name && userInfo.email && userInfo.picture;
    ...
    const newAccessToken = await new jose.SignJWT({ ...completeUserInfo, type: undefined, }) // Remove the refresh token specific fields from the access token
      .setProtectedHeader({ alg: "HS256" })
      .setExpirationTime(JWT_EXPIRATION_TIME)
      .setSubject(sub)
      .setIssuedAt(issuedAt)
    ...
    const newRefreshToken = await new jose.SignJWT({ ...completeUserInfo, jti, type: "refresh", })
      .setProtectedHeader({ alg: "HS256" })
    ...
    if (platform === "web") {
      ...
      response.headers.append(
        "Set-Cookie",
        `${REFRESH_COOKIE_NAME}=${newRefreshToken}; Max-Age=${REFRESH_COOKIE_OPTIONS.maxAge
