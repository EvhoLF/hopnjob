...
export async function POST(request: Request) {
  ...
  if (response.status === 200) {
    ...
    const refreshToken = await new jose.SignJWT({ sub, jti, type: "refresh", ...userInfo, })
      .setProtectedHeader({ alg: "HS256" })
    ...
    switch (errorData.error) {
      case "invalid_request":
        errorMessage = "The request is missing parameters or contains unsupported parameters";
        break;
      case "invalid_client":
        errorMessage = "Client authentication failed - invalid client ID, secret or redirect URI";
        break;
      case "invalid_grant":
        errorMessage = "Invalid or expired authorization code";
        break;
      case "unauthorized_client":
        errorMessage = "Client not authorized to use this authorization grant type";
        break;
    ...
    return Response.json({ error: errorMessage, details: errorData }, { status: 400 });
  } else {
    ...
    try {
      ...
      try { errorData = await response.text(); }
      catch { errorData = "Could not read response body"; }
    }
    return Response.json(
