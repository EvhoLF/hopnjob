...
export async function GET(request: Request) {
  try {
    ...
    if (!cookieHeader) { return Response.json({ error: "Not authenticated" }, { status: 401 }); }
    ...
    cookieHeader.split(";").forEach((cookie) => {
      ...
      if (trimmedCookie.includes("=")) {
        ...
        if (!cookies[cookieName]) { cookies[cookieName] = { value: value }; }
        else { cookies[cookieName].value = value; }
        ...
        const lastCookieName = Object.keys(cookies).pop();
        if (lastCookieName) { cookies[lastCookieName].expires = trimmedCookie.substring(8); }
      } else if (trimmedCookie.toLowerCase().startsWith("max-age=")) {
        // Handle Max-Age attribute
        const lastCookieName = Object.keys(cookies).pop();
        if (lastCookieName) { cookies[lastCookieName].maxAge = trimmedCookie.substring(8); }
      }
    });
    ...
    try {
      ...
      return Response.json({ ...verified.payload, cookieExpiration, });
    } catch (error) {
